...
func(b *bingoBoard) sumOfUnmarkedNumbers() (ret int64) {
	for i := 0;i<5;i++ {
		for j := 0; j <5;j++{
			if(!b.isNumberAlreadyMarked(b.board[i][j])){ret+=b.board[i][j]}
		}
	}
	return
}
...
func task2 (fileName string) {
	...
	for _,n := range numbersToProcess {
		for _,b := range boards {
			if(b.checkForBingo()){continue}
			b.markNumber(n)
			if(b.checkForBingo()){
				markedBoards = append(markedBoards,b)
			}
		}
		if(len(markedBoards)>=len(boards)){break}
	}
...
func main() {
	task1("testdata.txt")
	task1("day4-inputdata-1.txt")
	task2("testdata.txt")
	task2("day4-inputdata-1.txt")
}
